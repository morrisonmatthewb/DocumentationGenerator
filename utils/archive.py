...
from config.constants import SUPPORTED_EXTENSIONS, SUPPORTED_ARCHIVE_FORMATS
...
def extract_archive_to_temp_dir(uploaded_file, file_extension: str) -> Tuple[str, str]:
    ...
    try:
        ...
        if file_extension.lower() == '.zip':
            ...
            # For other formats, use patool which supports many archive types
            import patoolib
            patoolib.extract_archive(temp_archive_path, outdir=extraction_dir)
...
def extract_files_from_archive(uploaded_file, selected_extensions: Optional[List[str]] = None,
                              max_file_size_mb: int = 5) -> Dict[str, Dict[str, Any]]:
    """
    ...
    temp_dir, extraction_dir = extract_archive_to_temp_dir(uploaded_file, file_extension)
