...
from utils.pdf import convert_markdown_to_pdf
...
def sidebar_config() -> Dict[str, Any]:
    ...
    generate_dir_structure = st.sidebar.checkbox("Generate Directory Structure Visualization", value=True)
    ...
    with col2:
        if not files:
            ...
            with st.expander("Files found (organized by directory)"):
                ...
                root_files = [path for path, info in files.items() if not info.get('directory')]
                if root_files:
                ...
                for directory in sorted(directories):
                    ...
                    dir_files = [path for path, info in files.items() 
...
def display_download_options(documentation: Dict[str, str], key_suffix: str = ""):
    ...
    with col3:
        ...
        try:
            with st.spinner("Generating documentation file..."):
                output_data = convert_markdown_to_pdf(combined_docs, 
                                                   title="Project Documentation")
                
            # Determine if it's PDF or HTML
            first_bytes = output_data.read(10)
            output_data.seek(0)  # Reset position
